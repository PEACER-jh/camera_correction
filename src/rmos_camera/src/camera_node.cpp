...
CameraNode::CameraNode(const rclcpp::NodeOptions &options)
    : rclcpp::Node("daheng_camera", options), frame_id_(0), time_offset(0)
{
  ...
  camera_exp_info_srv_ = this->create_service<rmos_interfaces::srv::CameraInfo>(
      "/camera/exposure_info",
      std::bind(&CameraNode::AutoExposureCallBack, this, std::placeholders::_1, std::placeholders::_2));

  capture_thread_ = std::thread{[this]() -> void { this->capture_thread_lambda(); }};
  callback_handle_ =
      this->add_on_set_parameters_callback(std::bind(&CameraNode::ParamtersCallBack, this, std::placeholders::_1));
}
