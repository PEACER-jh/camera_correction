...
bool DahengCamera::isOpen() { return is_open_; }
bool DahengCamera::isRun() { return is_run_; }
bool DahengCamera::isInit() { return is_init_; }

bool DahengCamera::SensorOpen() {
  if (this->is_open_) return true;
  if (!this->SensorInit()) return false;
...
bool DahengCamera::SensorRun(cv::Mat &image) {
  ...
  VxInt32 DXstatus = DxRaw8toRGB24(pFrameBuffer_->pImgBuf, rgbImagebuf_, pFrameBuffer_->nWidth, pFrameBuffer_->nHeight,
  ...
  auto cost = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() -
                                                                    speed_test_start_begin_time)
                  .count();
